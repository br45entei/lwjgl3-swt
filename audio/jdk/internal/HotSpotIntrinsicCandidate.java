 ...
 * is intrinsified if the HotSpot VM replaces the annotated method with
 * hand-written
 * assembly and/or hand-written compiler IR -- a compiler intrinsic -- to
 * improve
 * performance. The {@code @HotSpotIntrinsicCandidate} annotation is internal to
 * the
 * Java libraries and is therefore not supposed to have any relevance for
 * application
 * code.
 *
 ...
 *
 * <ul>
 * <li>When modifying a method annotated with
 ...
 * <li>For some annotated methods, the corresponding intrinsic may omit some
 * low-level
 * checks that would be performed as a matter of course if the intrinsic is
 * implemented
 * using Java bytecodes. This is because individual Java bytecodes implicitly
 * check
 * for exceptions like {@code NullPointerException} and
 ...
 * appropriate, on all arguments to the intrinsic, and on other values (if any)
 * obtained
 * by the intrinsic through those arguments. The checks may be deduced by
 * inspecting
 * the non-intrinsic Java code for the method, and determining exactly which
 * exceptions
 * may be thrown by the code, including undeclared implicit
 * {@code RuntimeException}s.
 ...
 * <li>other validity checks on primitive values (e.g., for divide-by-zero
 * conditions)</li>
 ...
 * <li>reference casts (when formal parameters are {@code Object} or some other
 * weak type)</li>
 * </ul>
 *
 * </li>
 ...
 * to all non-static methods. If a non-static method is an intrinsic, the
 * receiver
 * value does not need a null check, but (as stated above) any values loaded by
 * the
 * intrinsic from object fields must also be checked. As a matter of clarity, it
 * is
 * better to make intrinsics be static methods, to make the dependency on
 * {@code this}
 * clear. Also, it is better to explicitly load all required values from object
 * fields before entering the intrinsic code, and pass those values as explicit
 * arguments.
 * First, this may be necessary for null checks (or other checks). Second, if
 * the
 * intrinsic reloads the values from fields and operates on those without
 * checks,
 * race conditions may be able to introduce unchecked invalid values into the
 * intrinsic.
 * If the intrinsic needs to store a value back to an object field, that value
 * should be
 * returned explicitly from the intrinsic; if there are multiple return values,
 * coders
 * should consider buffering them in an array. Removing field access from
 * intrinsics
 * not only clarifies the interface with between the JVM and JDK; it also helps
 * decouple
 * the HotSpot and JDK implementations, since if JDK code before and after the
 * intrinsic
 * manages all field accesses, then intrinsics can be coded to be agnostic of
 * object
 * layouts.</li>
 *
 ...
 * and that all possible call sequences that result in calling the intrinsic
 * contain
 ...
 * the VM's list of intrinsics are annotated with
 * {@code @HotSpotIntrinsicCandidate}
